	...
	public function create_uri($data = '', $ids = false)
	{
		...
		if (is_array($data)) {
			if (!empty($data[$this->field])) {
				return $this->_check_uri($this->create_slug($data[$this->field]), $ids);
			} elseif (!empty($data[$this->title])) {
				return $this->_check_uri($this->create_slug($data[$this->title]), $ids);
			}
		}
		...
		if ($ids) {
			foreach ($this->id as $i => $id) {
				if (isset($ids[$i]) && !empty($ids[$i])) {
					$this->qb->where($id . ' !=', $ids[$i]);
				}
			}
		}

		if ($this->qb->countAllResults() > 0) {
			return $this->_check_uri($uri, $ids, ++$count);
		} else {
			return $new_uri;
		}
	}
